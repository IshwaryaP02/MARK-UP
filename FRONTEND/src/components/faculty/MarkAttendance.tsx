...
export const MarkAttendance: React.FC = () => {
  ...
  const activeSubjectSlot = useMemo(() => {
    if (!selectedSubject) return undefined;
    const todayDay = new Date().toLocaleDateString('en-US', { weekday: 'long' });
    return timetable.find(
      (t) => t.subjectId === selectedSubject.id && t.day === todayDay && t.periodNumber === currentPeriod
    );
  }, [timetable, selectedSubject, currentPeriod]);

  const scheduledSubjectSlotsToday = useMemo(() => {
    if (!selectedSubject) return [];
    const todayDay = new Date().toLocaleDateString('en-US', { weekday: 'long' });
    return timetable.filter((t) => t.subjectId === selectedSubject.id && t.day === todayDay);
  ...
  // Strict enforcement: if the current active period doesn't match the selected subject's scheduled slot
  if (!activeSubjectSlot) {
    const isPast = scheduledSubjectSlotsToday.some((s) => s.periodNumber < currentPeriod);
    const isFuture = scheduledSubjectSlotsToday.some((s) => s.periodNumber > currentPeriod);

    return (
      <div className="space-y-6">
        <div className="pb-2 border-b border-[#E2E8F0] dark:border-zinc-800 flex items-center gap-4">
          <button
            onClick={() => setActiveScreen('my_classes')}
          ...
          <h2 className="text-lg font-bold text-[#0F172A] dark:text-zinc-100 tracking-tight flex items-center gap-2">
            <Lock className="w-5 h-5 text-rose-500" /> Attendance Locked
          </h2>
        </div>
        <div className="p-8 bg-white dark:bg-[#0A0A0A] border border-rose-200 dark:border-rose-900 rounded-2xl text-center space-y-4 shadow-sm">
          <Lock className="w-12 h-12 text-rose-500 mx-auto" />
          <div>
            <p className="text-sm font-bold text-rose-600 dark:text-rose-400">Time Window Restricted</p>
            <p className="text-xs text-[#000000] dark:text-[#64748B] dark:text-zinc-400 mt-2 max-w-md mx-auto leading-relaxed">
              Attendance for <strong>{selectedSubject.code}</strong> can only be marked or edited during its scheduled period.
              <br/><br/>
              {scheduledSubjectSlotsToday.length > 0 ? (
                <>
                  Scheduled today for Period(s): {scheduledSubjectSlotsToday.map(s => s.periodNumber).join(', ')}<br/>
                  Current active period is: <strong>Period {currentPeriod}</strong>
                </>
              ) : (
                <>This subject is not scheduled for today.</>
              )}
            </p>
          </div>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  >
                    <Avatar
                      name={studentObj?.name || entry.studentId}
